...
func FindUserByNameAndPwd(name string, password string) (*models.UserBasic, error) {
	...
	if tx := global.DB.
		Where("name = ? and pass_word=?", name, password).
		First(&user); tx.RowsAffected == 0 {
	...
	if tx := global.DB.
		Model(&user).
		Where("id = ?", user.ID).
		Update("identity", temp); tx.RowsAffected == 0 {
...
func FindUserID(ID uint) (*models.UserBasic, error) {
	user := models.UserBasic{}
	if tx := global.DB.
		Where(ID).First(&user); tx.RowsAffected == 0 {
		return nil, errors.New("未查询到记录")
	}
	return &user, nil
}
